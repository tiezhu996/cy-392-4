...
export async function readViewHistory(): Promise<ViewHistory[]> {
  const db = await openDb();
  return new Promise((resolve) => {
    const req = db.transaction(STORE_VIEW_HISTORY).objectStore(STORE_VIEW_HISTORY).getAll();
    req.onsuccess = () => resolve(req.result as ViewHistory[]);
  });
}

export async function writeViewHistory(history: ViewHistory[]) {
  const db = await openDb();
